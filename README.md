...
Sku ──────────────────────────── one SKU = one product variant (e.g. DDR5-16G, NAND-1T)
 ├── Lot                         physical batch — lot_code, quantity (immutable), available_qty, status, received_at
...
Every time units move — committed, released, or soft-held — an `AllocationEvent` row is written inside the same transaction. The table is append-only (all FKs use `RESTRICT` delete behaviour so no event can be silently removed when an order or line is deleted). Phase 20 adds a nullable `LotId` FK: every `allocation_committed` event written by the lot-aware allocation path carries the specific lot that was consumed. Phase 22 adds `LotStatusSnapshot`: the lot's status at the moment the allocation was committed, captured inside the same transaction so it is immutable and independent of the lot's current state. `EventType` is one of five values:
...
- **Lot Traceability Queries** — four read-only Dapper endpoints that close the traceability loop built across Phases 18–20: `GET /lots/{id}/allocations` returns a paginated history of every `allocation_committed` event for a lot (which orders consumed it, how many units each, ordered `occurredAt` descending); `GET /lots/{id}/events` returns the full lifecycle audit trail in chronological order (quarantine, release, and scrap entries from `lot_events`); `GET /orders/{id}/lots` returns the lots consumed to fill an order, with per-lot `quantityConsumed` aggregated via `GROUP BY` across multiple allocation runs — returns an empty list (not 404) for orders filled from the fallthrough on-hand path; each entry carries both `lotStatusAtAllocation` (the lot's status when the order was committed, snapshotted in Phase 22) and `lotStatusNow` (current status at query time); `GET /skus/{id}/lots/snapshot` returns a live inventory picture — `on_hand`, a per-status aggregate summary (`lotCount`, `totalAvailableQty`), and the full individual lot list newest-received-first — sent in a single round trip via `QueryMultipleAsync` across three `SELECT` statements
- **Lot Status Snapshot** — `allocation_events.lot_status_snapshot` column (`VARCHAR(20)`, nullable) captures the lot's status at the moment an `allocation_committed` event is written, inside the same transaction as the allocation itself; the snapshot is immutable for the lifetime of the row; rows written before Phase 22 and fallthrough-path events (no `lot_id`) have `null`; `GET /orders/{id}/lots` surfaces both `lotStatusAtAllocation` (from `MIN(lot_status_snapshot)` — safe because all events for a given `(order_id, lot_id)` group were written in the same run against the same lot) and `lotStatusNow` (from `lots.status`), enabling containment reports that distinguish material shipped clean from material shipped under a hold
...
- **Cancellation & Release** — `POST /orders/{id}/cancel` transitions the order to `cancelled` and atomically restores `on_hand` from each line's `allocated_qty`; for lot-tracked orders it also reads `allocation_committed` events to restore each lot's `available_qty` (and transitions `Depleted` lots back to `Available`); cancel uses the same lock order as allocate (SKU first, then lots in `id` ascending order) to prevent concurrent allocation/cancel deadlocks
...
├── Domain/Entities/
│   ├── Sku.cs                              # AllocateUnits(), ReleaseUnits(), ReceiveLot(qty), HoldForQuarantine(qty), RestoreFromQuarantine(qty); version concurrency token; Lots navigation collection
│   ├── Lot.cs                              # id, sku_id, lot_code, quantity (immutable), available_qty (mutable), status, received_at, created_at; Quarantine(), Release(), Scrap() return on_hand delta; Consume(qty) decrements available_qty (→ Depleted at 0); Restore(qty) increments available_qty (→ Available if Depleted)
│   ├── LotEvent.cs                         # id, lot_id, sku_id, event_type, quantity_affected, notes, occurred_at — immutable audit row written by each status transition
...
│   ├── Reservation.cs
│   ├── AllocationEvent.cs                  # Immutable ledger row — EventType, OrderId, OrderLineId, SkuId, Quantity, OccurredAt; LotId (Guid?, nullable) added Phase 20 — carries the lot consumed on allocation_committed events from the lot-aware path; LotStatusSnapshot (string?, nullable) added Phase 22 — lot's status at commit time, captured inside the same transaction
...
│   ├── LotStatus.cs                        # Available | Quarantined | Depleted | Scrapped; ToDbString() / FromDbString() extensions
│   ├── LotEventType.cs                     # Quarantined | Released | Scrapped; ToDbString() / FromDbString() extensions
...
│   ├── LotRecords.cs                       # ReceiveLotRequest (LotCode, Quantity, ReceivedAt?) / LotResponse / LotStatusTransitionRequest (Notes?) / LotAllocationHistoryEntry / LotEventHistoryEntry / OrderLotProvenanceEntry (LotStatusAtAllocation? + LotStatusNow replacing single LotStatus field — Phase 22) / LotStatusSummary / LotSnapshotEntry / SkuLotSnapshotResponse
...
│   ├── AllocationService.cs                # AllocateAsync (TX + FOR UPDATE on SKUs then lots in id-asc order); lot-aware path: FIFO lot selection, Lot.Consume(), per-lot AllocationEvent with lot_id, LotAllocations on result; fallthrough path for SKUs with no Available lots; AllocateCappedAsync (private, per-SKU cap overrides); two-pass RunPriorityAllocationAsync; GetAvailabilityAsync; GetEventsAsync; emits AllocationCommitted events + outbox
...
│   └── IdempotencyCleanupJob.cs            # BackgroundService — deletes expired 'complete' records and stuck 'processing' records
├── Data/
│   ├── AllocationDbContext.cs              # EF Core context — entity configs, indexes, CHECK constraints; Lots + LotEvents + CustomerContracts + AllocationRuns DbSets
...
│                                           # (lot_events, customer_contracts, allocation_runs, and all dependent tables deleted in FK order);
...
|---|---|---|---|
| POST | `/skus/{skuId}/lots` | `warehouse-ops` | Receive a new lot for a SKU |
| GET | `/skus/{skuId}/lots` | any-auth | List lots for a SKU, ordered by `receivedAt` descending |
| GET | `/lots/{id}` | any-auth | Get a lot by ID |
...
| GET | `/orders/{orderId}/lots` | any-auth | Lots consumed to fill an order, with quantities and both allocation-time and current lot status |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `lotCode` | string | yes | Physical batch identifier — globally unique (max 64) |
| `quantity` | int | yes | Units received in this lot (must be > 0) |
| `receivedAt` | DateTimeOffset? | no | When the lot was physically received; defaults to `UtcNow` if omitted |

The lot insert, `sku.on_hand` increment, and outbox `lot.received` message are committed atomically. Returns 404 if the SKU does not exist. Returns 422 if `lotCode` already exists or `quantity ≤ 0`.

**POST `/lots/{id}/quarantine|release|scrap` body (optional):**

| Field | Type | Required | Description |
|---|---|---|---|
| `notes` | string? | no | Free-text reason (max 500); stored in the `LotEvent` audit record |

All three transition endpoints lock the SKU row first then the lot row (`FOR UPDATE`) in that order — consistent with the Phase 4 deadlock prevention strategy. A `LotEvent` row and an outbox message are written inside the same transaction as the status change.

**GET `/skus/{skuId}/lots` query params:**

| Param | Default | Description |
|---|---|---|
| `status` | — | Filter: `available`, `quarantined`, `depleted`, or `scrapped` |
...
|---|---|---|
| `id` | Guid | Lot ID |
| `skuId` | Guid | SKU this lot belongs to |
| `skuCode` | string | SKU code |
| `lotCode` | string | Physical batch identifier |
| `quantity` | int | Total units received — immutable after intake |
| `availableQty` | int | Units currently available; decremented by allocation and quarantine |
...
| `lotStatusAtAllocation` | string? | The lot's status when this order was committed — snapshotted inside the allocation transaction; `null` for orders allocated before Phase 22 |
| `lotStatusNow` | string | The lot's current status at query time |
| `receivedAt` | DateTime | UTC timestamp of physical receipt |

`lotStatusAtAllocation` and `lotStatusNow` diverge when a lot is quarantined or scrapped after an order ships — the snapshot preserves what the system knew at commit time, enabling containment reports that distinguish material shipped clean from material shipped under a hold.
...
| Code | Meaning |
|---|---|
| 201 | Lot received and `on_hand` updated |
| 200 | OK (GET endpoints and status transition endpoints) |
| 404 | SKU or lot not found |
| 409 | Invalid status transition (e.g. quarantine a lot that is already quarantined or scrapped) |
| 422 | Duplicate `lotCode`, `quantity ≤ 0`, or missing required fields |
...
| `lotAllocations` | array? | Per-lot breakdown — `null` for SKUs with no lots (fallthrough path) and when zero units were allocated; present when lot-aware allocation consumed at least one lot |

Each `lotAllocations` entry:

| Field | Type | Description |
|---|---|---|
| `lotId` | Guid | Lot ID |
| `lotCode` | string | Physical batch identifier |
| `quantityConsumed` | int | Units taken from this lot in this allocation run |
...
lotCode       string  required, unique (max 64) — unique index idx_lots_lot_code
quantity      int     total units received — immutable after intake (CHECK quantity > 0)
availableQty  int     units currently available; starts equal to quantity (CHECK available_qty >= 0; CHECK available_qty <= quantity)
status        string  available | quarantined | depleted | scrapped
receivedAt    DateTimeOffset
createdAt     DateTimeOffset
```

Domain methods:
- `Quarantine()` / `Release()` / `Scrap()` — return the `on_hand` delta for the caller to apply to the SKU
- `Consume(qty)` — decrements `available_qty` by `qty`; transitions to `Depleted` when `available_qty` reaches 0; throws if the lot is not `Available` or `qty` exceeds `available_qty`
- `Restore(qty)` — increments `available_qty` by `qty`; transitions `Depleted` back to `Available`; throws for `Quarantined` or `Scrapped` lots (those statuses are operator-set and are not reversed by a business cancellation)

Indexes:
- `idx_lots_lot_code` — unique on `lot_code` (global uniqueness constraint)
- `idx_lots_sku_status_received` — composite on `(sku_id, status, received_at)` — serves the FIFO lot selection query in `AllocateAsync` (`WHERE sku_id = ANY(@ids) AND status = 'available' ORDER BY id FOR UPDATE`, then sorted by `received_at` in memory after locks are held)

### LotEvent

```
id                Guid    PK (gen_random_uuid())
lotId             Guid    FK → Lot (restrict)
skuId             Guid    FK → Sku (restrict)
eventType         string  quarantined | released | scrapped (max 32)
quantityAffected  int     units removed from or restored to sku.on_hand (0 when scrapping from Quarantined)
notes             string? free-text reason provided by the operator (max 500)
occurredAt        DateTimeOffset
```

`RESTRICT` on both FKs — lot events are an immutable audit ledger; a lot cannot be deleted once it has events. `quantityAffected` records the exact `on_hand` impact of the transition: for a scrap from `quarantined` state this is 0, since the impact already occurred at quarantine time.

Indexes:
- `idx_lot_events_lot_id` — primary query pattern: all events for a given lot (Phase 21 traceability endpoint)
- `idx_lot_events_sku_id` — secondary: audit queries across all lots for a single SKU
...
lotId               Guid?   FK → Lot (restrict, nullable) — set on allocation_committed events from the lot-aware path; null for fallthrough-path events and all non-allocation event types
lotStatusSnapshot   string? lot's status at the moment this event was committed — populated for lot-aware allocation_committed events; null for fallthrough-path events, all non-allocation event types, and rows written before Phase 22 (max 20)
occurredAt          DateTimeOffset
```

All FKs use `RESTRICT` — rows cannot be deleted if they have event history. The table is append-only; there is no update or delete path in the application.

Indexes: `orderId` (primary query: all events for one order), `skuId` (cross-order audit queries by SKU), `occurredAt` (chronological range queries), `lotId` partial index `WHERE lot_id IS NOT NULL` (Phase 21 traceability query: which orders consumed from a given lot)
...
| 22 | Lot status snapshot at allocation time — closes containment-reporting gap identified in `design-decisions.md §1`; `allocation_events.lot_status_snapshot` column (`VARCHAR(20)`, nullable, migration `AddLotStatusSnapshotToAllocationEvents`); `AllocationEvent` entity gains `LotStatusSnapshot` property and constructor parameter; `AllocateAsync` lot-aware path passes `lot.Status.ToString()` as the snapshot — written inside the same `SaveChangesAsync` call as the allocation so it is always consistent; fallthrough-path events (no `lot_id`) leave the column null; `OrderLotProvenanceEntry` record splits the single `LotStatus` field into `LotStatusAtAllocation` (string?, from `MIN(ae.lot_status_snapshot)`) and `LotStatusNow` (string, from `lots.status`); `GetOrderLotsAsync` Dapper query updated accordingly; `LotTests.cs` 9c-v updated to assert both fields; new integration test 10a-i verifies the divergence directly (allocate available lot, quarantine it, assert `LotStatusAtAllocation=available` and `LotStatusNow=quarantined`) | Done |
